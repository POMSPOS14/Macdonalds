...
public class ProductRepositoryJdbcImpl implements ProductRepository {
    ...
    public List<ProductModel> getAll() {
        try {
            return template.queryForList(ds, "SELECT id, name, price, quantity, image_url, category FROM products;", mapper);
        } catch (SQLException e) {
            throw new DataAccessException(e);
        }
    }
    ...
    public Optional<ProductModel> getById(int id) {
        try {
            return template.queryForObject(ds, "SELECT id, name, quantity, price, image_url, category FROM products WHERE id = ?;", stmt -> {
                stmt.setInt(1, id);
                return stmt;
            }, mapper);
        } catch (SQLException e) {
    ...
    public void save(ProductModel model) {
        try {
            if (model.getId() == 0) {
                int id = template.<Integer>updateForId(ds, "INSERT INTO products(name, price, quantity, image_url, category) VALUES (?, ?, ?, ?, ?);", stmt -> {
                    int nextIndex = 1;
                ...
                template.update(ds, "UPDATE products SET name = ?, price = ?, quantity = ?, image_url = ?, category = ? WHERE id = ?;", stmt -> {
                    int nextIndex = 1;
                    stmt.setString(nextIndex++, model.getName());
                    stmt.setInt(nextIndex++, model.getPrice());
                    stmt.setInt(nextIndex++, model.getQuantity());
